...
"""
import numpy as np
from .utils import helper_diis, einsums_contract
import time
...
class ccwfn(object):
    ...
    def __init__(self, scf_wfn, aux_bas, **kwargs):
        ...
        self.contract = einsums_contract()
    ...
    def solve_cc(self, e_conv=1e-7, r_conv=1e-7, maxiter=100, max_diis=8, start_diis=1):
        ...
        for niter in range(1, maxiter + 1):
            ...
            # Build DIIS error vector as (Tnew - Told), like Psi4's RT1/RT2
    ...
    def build_tau_ijab(self, t1):
        contract = self.contract
        tmp = contract('ia,jb->iajb', t1, t1, alpha=0.5)
        return tmp.swapaxes(1,2)

    def build_Tau_ijab(self, t1, t2):
        contract = self.contract
        tmp = contract('ia,jb->iajb', t1, t1)
        return t2 + tmp.swapaxes(1,2)
    ...
    def build_Xijab(self, t2, Fae, Fmi, tiaQ_prime, tiaQ, taiQ, BovQ):
        ...
        X = contract('mi,mjab->ijab', Fmi, t2, alpha=-1.0, beta=1.0, out=X)
        tmp = contract('iaQ,jbQ->iajb', tiaQ_prime, BovQ)
        X += tmp.swapaxes(1,2)
        tmp = contract('aiQ,jbQ->aijb', taiQ, tiaQ)
        X -= tmp.swapaxes(0,1).swapaxes(1,2)
        #print("time taken to compute Xijab: ", time.time() - start)
    ...
    def build_WmnijT2(self, BooQ, BovQ, tijQ, tau_ijab):
        ...
        X = contract("imQ,jnQ->imjn", tijQ, BooQ).swapaxes(0,1).swapaxes(2,3).swapaxes(1,2)
        W += X + X.swapaxes(0,1).swapaxes(2,3)
        #W += X.swapaxes(0,1).swapaxes(1,3).swapaxes(2,3)
        V = contract("meQ,nfQ->menf", BovQ, BovQ).swapaxes(1, 2)
        W += contract("mnef,ijef->mnij", V, tau_ijab)
        W1 = contract("mnij,mnab->ijab", W, tau_ijab)
        #print("time taken to compute WmnijT2: ", time.time() - start)
    ...
    def build_WmbejT2(self, BovQ, BooQ, BvvQ, tijQ, tabQ, tiaQ_prime, TiaQ, t2, u_ijab):
        ...
        # W'(me,jb)
        W2 = contract('mjQ,ebQ->mjeb', BooQ, BvvQ).swapaxes(1,2)
        tmp = contract('jmQ,beQ->jmbe', tijQ + BooQ, tabQ)
        W2 -= tmp.swapaxes(0,1).swapaxes(2,3).swapaxes(1,2)
        tmp = contract('jmQ,beQ->jmbe', tijQ, BvvQ)
        W2 += tmp.swapaxes(0,1).swapaxes(2,3).swapaxes(1,2)
        W2 = contract('mfne,njbf->mejb', V, t2, alpha=-0.5, beta=1.0, out=W2)
        # t_ij^ab <- P_ij^ab[0.5 * C(ia,jb) + C(ib,ja)]
        C = contract('miae,mejb->iajb', t2, W2, alpha=-1.0).swapaxes(1,2)
        tmpC = 0.5 * C + C.swapaxes(2,3)
        W_T2 = tmpC + tmpC.swapaxes(0,1).swapaxes(2,3)
        # t_ij^ab <- P_ij^ab Dijab
        D = contract('imae,mejb->iajb', u_ijab, 2 * W1 - W2, alpha=0.5).swapaxes(1,2)
            ...
            for b in range(nv):
                ...
                for i in range(no):
                    for j in range(no):
                        ...
                        W[i, j, a, b] = val
        #print("time taken to compute WabefT2_packed: ", time.time() - start)
    ...
    def cc_energy(self, o, v, F, BovQ, t1, t2):
        ...
        V = contract('iaQ, jbQ->iajb', BovQ, BovQ).swapaxes(1,2)
    ...
    def triples_correction_1(self, o, v, F, BovQ, BooQ, BvvQ, t1, t2, Dijkabc):
        ...
        # Build W
        W = contract('iaeb,jkec->iabjkc', Vovvv, t2).swapaxes(1,3).swapaxes(2,4)
        W -= contract('imab,mjkc->iabjkc', t2, Vooov).swapaxes(1,3).swapaxes(2,4)
        ...
        Vijkabc += contract('ia,jbkc->iajbkc', t1, Vovov).swapaxes(1,2).swapaxes(2,4).swapaxes(3,4)
        Vijkabc += contract('iakc,jb->iakcjb', Vovov, t1).swapaxes(1,4).swapaxes(3,4).swapaxes(4,5)
        Vijkabc += contract('iajb,kc->iajbkc', Vovov, t1).swapaxes(1,2).swapaxes(2,4).swapaxes(3,4)
    ...
    def triples_correction_2(self, o, v, F, BovQ, BooQ, BvvQ, t1, t2, Dijkabc):
        ...
        V = W.copy()
        V += contract('ia,jbkc->iajbkc', t1, Vovov).swapaxes(1, 2).swapaxes(2, 4).swapaxes(3, 4)
        V += contract('iakc,jb->iakcjb', Vovov, t1).swapaxes(1, 4).swapaxes(3, 4).swapaxes(4, 5)
        V += contract('iajb,kc->iajbkc', Vovov, t1).swapaxes(1, 2).swapaxes(2, 4).swapaxes(3, 4)
