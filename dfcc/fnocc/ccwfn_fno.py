"""
ccwfn_fno.py: CCSD(T) T-amplitude solver (opt_einsum (default))
"""
import numpy as np
...
class ccwfn(object):
    ...
    def __init__(self, scf_wfn, aux_bas_scf, aux_bas_cc, **kwargs):
        ...
        self.aux_bas_cc = aux_bas_cc  # defaut for CC: "RI"
        ...
        # NOTE: CHECK CONSISTENCY IF FREEZE VIRTUAL ORBITALS
        ...
        self.contract = opt.contract # use opt_einsum
        ...
        if self.nat_orbs == True:
            ...
            # Initial amplitudes (truncated virtual space)
    ...
    def residuals(self, t1, t2):
        ...
        # Permutation of 4-index two-electron integrals, Eq. 17
    ...
    def build_Fock_LR(self, h_ao, C_L, C_R, C_L_act, C_R_act, BooQ, BovQ, BvvQ, BvoQ):
        """
        Build t1-transformed Fock matrix blocks in active space.

        Parameters
        ----------
        h_ao : NumPy array
            AO core Hamiltonian.
        C_L, C_R : NumPy arrays
            T1-dressed MO coefficient matrices.
        C_L_act, C_R_act: NumPy arrays
            Slices of C_L and C_R in active space respectively.
        BooQ, BvvQ, BovQ, BvvQ: NumPy arrays
            T1-transformed DF integrals in full MO space
        Returns
        -------
        ...
        # hLR: (nmo, nmo)
        hLR = contract("pm,pq,qn->mn", C_L_act, h_ao, C_R_act)
    ...
    def build_L_iajb(self, BovQ):
        ...
        #tmp2 = contract('ibQ,jaQ->iajb', BovQ, BovQ)
    ...
    def B_ijab(self, t2, BooQ, BovQ):
        ...
        tmp1 += contract('ijcd,kcld->kilj', t2, tmp2)
    ...
    def C_ijab(self, t2, BooQ, BovQ, BvvQ):
        ...
        V1 -= 0.5 * tmp
        Cijab = -contract("kjbc,kiac->ijab", t2, V1)
    ...
    def D_ijab(self, L_iajb, L_aijb, u_ijab):
        ...
        tmp += 1 / 2 * contract('ilad,ldkc->aikc', u_ijab, L_iajb)

        return 1 / 2 * contract('jkbc,aikc->ijab', u_ijab, tmp)
    ...
    def E_ijab(self, t2, u_ijab, Fae, BovQ):
        ...
        tmp -= contract('klbd,ldkc->bc', u_ijab, tmp1)

        return contract('ijac, bc->ijab', t2, tmp)

    # Gijab = - sum_k (t_ikab [Fkj - sum_lcd(u_ljcd [sum_Q (BkdQ BlcQ)])])
    # note: typo in Gijab = - sum_k (t_ikab [Fkj + (not -) sum_lcd(u_ljcd [sum_Q (BkdQ BlcQ)])])
    def G_ijab(self, t2, u_ijab, Fij, BovQ):
        contract = self.contract
        tmp = Fij.copy()
        tmp1 = contract('kdQ,lcQ->kdlc', BovQ, BovQ)
        tmp += contract('ljcd,kdlc->kj', u_ijab, tmp1) #? +/-

        return - 1.0 * contract('ikab,kj->ijab', t2, tmp)
